# Accept either a binary stream (output of open) or table (output of ls)
# with-env {NU_LOG_LEVEL:DEBUG } { use std log }

use std log
# Expecting a list of files generated by `ls`.
# For each row, update the null rows with information gathered from ffprobe.
# Get a parsed json output from ffprobe and put it in file_info so we can manipulate it.
# The resulting table may contain one or two streams. If it's a video file, there's a 
# video and audio stream. The audio stream contains most of the information we're trying to get.
export def "media info from ls" []: table<name: string> -> table<name: string, audio_codec: string, channels: int, media_type: string, sample_rate: int> {
    $in
    | insert audio_codec null
    | insert channels null
    | insert media_type null
    | insert sample_rate null
    | each {|row|
        let file_info: table = (
            open $row.name
            | ^ffprobe -v quiet -print_format json -show_streams -i pipe:0
            | from json
            | get streams
        )
        log debug $"file_info is ($file_info | describe)"

        $row
        | update media_type ($file_info | first | get codec_type)
        | update sample_rate ($file_info
            | where {|x| $x.codec_type? == 'audio'} 
            | get sample_rate?.0
            | into int
        )
        | update channels ($file_info
            | where {|x| $x.codec_type? == 'audio'} 
            | get channels?.0 
            | into int
        )
        | update audio_codec ($file_info 
            | where {|x| $x.codec_type? == 'audio'} 
            | get codec_name?.0 
            | into string
        )
    }
}

# Converts media files to MP3 format.
# 
# Use as part of a pipeline. Feed it a table
# of audio or video files via `ls`, and it
# adds a new column of binary data.
#
# Save the binary data using `to file`.
#
# Examples:
#   ls *.aac | into mp3 | to file
#   ls *.mp4 | into mp3 | to file
#
# Available profiles:
# - voice_low_quality
#   8 kHz sample rate, 32 kbps bit rate
# - voice_normal
#   16 kHz sample rate, 64 kbps bit rate (default)
# - music
#   44.1 kHz sample rate, 192 kbps bit rate
export def "into mp3" [
    profile?: string    # The profile name to use.
]: table<name: string> -> table<name: string, mp3: binary> {
    let column_name: string = 'mp3'    
    let mp3_profiles: table = [
        {profile_name: "voice_low_quality", sample_rate: 8000, bit_rate: 32},
        {profile_name: "voice_normal", sample_rate: 16000, bit_rate: 64},
        {profile_name: "music", sample_rate: 41100, bit_rate: 192}
    ] 
    
    let profile_name: string = if ($profile | is-empty) { 
        'voice_normal' 
    } else { $profile }

    let settings: record = $mp3_profiles | where {|x| $x.profile_name == $profile_name }

    $in 
    | media info from ls
    | insert $column_name null
    | each {|file|
        let display_options = ['-v' 'quiet']
        let input_options: list = ['-i' 'pipe:']
        let output_options =  [
            '-ar' ...$settings.sample_rate 
            '-ac' $file.channels
            '-ab' ([...$settings.bit_rate  'k'] | str join '')
            '-f' 'mp3'
            'pipe:'
        ]

        let final_command: list<any> = [
            $display_options $input_options $output_options
        ] 
        | flatten
        # log info $"\nfinal command: ($final_command | describe)"
        # print $"|DEBUG| Constructed command: ($final_command)"

        $file 
        | update $column_name (
            open $file.name
            | ^ffmpeg ...$final_command
            | into binary
        )
        # print $"|INFO| File converted. Generated wav data for ($file.name)."
    }  
}

# Converts media files to MP3 format.
# 
# Use as part of a pipeline. Feed it a table
# of audio or video files via `ls`, and it
# adds a new column of binary data.
#
# Save the binary data using `to file`.
#
# Examples:
#   ls *.aac | into wav | to file
#   ls *.mp4 | into wav | to file
export def "into wav" []: table<name: string> -> table<wav: binary> {
    $in 
    | media info from ls
    | insert wav null
    | each {|file|
        let display_options: list = ['-v' 'quiet']
        let input_options: list = ['-i' 'pipe:']
        let video_flags: list = (if ($file.media_type == 'video') {
            ['-vn' '-f' 'mov']
        } else { 
            [] 
        })

        let output_options: list =  [
            '-ar' '16000' 
            '-ac' '1'
            '-c:a' 'pcm_s16le'
            '-f' 'wav'
            'pipe:'
        ]

        let final_command: list = [
            $display_options $input_options $video_flags $output_options
        ]
        | flatten
        # print $"|DEBUG| Constructed command: ($final_command)"

        $file 
        | update wav (
            open $file.name
            | (^ffmpeg ...$final_command)
            | into binary
        )
        # print $"|INFO| File converted. Generated wav data for ($file.name)."
    }   
} 

# Saves data from table columns to files.
#
# Use with other `into <media>` conversion
# commands.
# Works on data stored in table columns and
# saves the media under the same filename.
#
# Examples:
#     ls *.aac | into mp3 | to file
#     ls *.mp4 | into wav | to file
export def "to file" []: table -> list<string> {
    $in 
    | each {|file|
        let is_mp3: bool = $file | columns | any {|x| $x == 'mp3' }
        let is_wav: bool = $file | columns | any {|x| $x == 'wav' }
        let is_transcript: bool = $file | columns | any {|x| $x == 'transcript'}

        let output_format: record = if ($is_mp3 == true) {
            {extension: 'mp3', column: ['mp3']}
        } else if ($is_wav == true) { 
            {extension: 'wav', column: ['wav']}
        } else if ($is_transcript == true) {
            {extension: 'txt', column: ['transcript' 'text']}
        }
        let output_file = ($file.name
        | path parse
        | [ $in.stem '.' $output_format.extension]
        | str join ""
    )

        if ($output_file | path exists) {
            log info $"File exists. Skipped saving ($output_file)."
        }
        if (not ( $output_file | path exists )) {
            log debug $"File saved. Data written to file ($output_file)"

            $file 
            | if ($is_transcript == true) { 
                ( get $output_format.column.0 | get $output_format.column.1 | str join ' ' )
            } else { 
                get $output_format.column.0
            }
            | save $output_file
        }
        $output_file
    }
} 

# Convert media files into text transcripts.
#
# Use as part of a media file conversion pipeline.
#
# Examples:
#   ls *.{mp3,mp4} | into wav | into transcript | to file
export def "into transcript" []: table<wav: binary> -> table<timestamps: record, offsets: record, text: string> {
    # TODO:
    #  - Allow the user to 
    let selected_profile = 'windows'
    let device_profiles = [[device, model_filepath, threads];
        ['windows' 'F:/M/models-speech-to-text/ggml-large-v3-turbo-q5_0.bin' '10']
        ['linux' '~/M/models-ggml.bin' '2']
    ]
    let device_setting = $device_profiles | where device == $selected_profile

    # TODO: Let the user pass a record of device settings, or just use the device_settings as default
    let setting = { 
        model_filepath: ($device_setting | get model_filepath).0,
        threads: ($device_setting | get threads).0 
    }

    # TODO Put the files in the system tmp folderpath instead? with -t
    let file_list = ($in 
        | reject wav     
        # | insert wav_filepath { (mktemp -t XXXXXX.wav) }
        | insert wav_filepath { (mktemp XXXXXX.wav) }
        | insert transcript_filepath { (mktemp XXXXXXX.json) }
        # | insert transcript_filepath { (mktemp -t XXXXXXX.json) }
        | insert transcript null
    )
    

    # Extracate binary wav data from table for processing
    # with whisper, since it won't use pipes.
    $in
    | enumerate
    | each {|row|
        let index: int = $row.index
        let filepath = $file_list | get $index | get wav_filepath
        
        $row.item | get wav | save --force $filepath
    }

    # Transcribe using whisper.cpp
    $file_list
    | update transcript {|file|
        let output_filepath = ($file | get transcript_filepath | path parse | get stem)

        # TODO: Convert the long string of whisper arguments into an array
        let whisper_arguments = [
            '--no-prints'
            '--threads' $setting.threads
            '-fa'
            '--language' 'en'
            '-m' $setting.model_filepath
            '--output-json'
            '--output-file' $output_filepath
            '-d' '10000'
            '-f' ($file | get wav_filepath)
        ] | flatten

        do {
            log debug $"whisper arguments: ($whisper_arguments | str join ' ')"
            ^whisper ...$whisper_arguments
            | complete
        }
        | if ($in.exit_code == 0) {
            log debug $"transcript filepath: ($file | get transcript_filepath)"

            open --raw ($file | get transcript_filepath)
            | from json
            | get transcription
        }
    } | cleanup-temporary-files ['wav_filepath' 'transcript_filepath']
            
}

def cleanup-temporary-files [column_names: list] {
    $in | each {|row|
        rm ($row | get transcript_filepath)
        log debug $"Deleted: ($row | get transcript_filepath)"

        rm ($row | get wav_filepath)
        log debug $"Deleted: ($row | get wav_filepath)"
        }
    $in
}